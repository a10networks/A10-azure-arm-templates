...
$ParamData = Get-Content -Raw -Path ARM_TMPL_3NIC_2VM_HA_PARAM.json | ConvertFrom-Json -AsHashtable
if ($null -eq $ParamData) {
    Write-Error "ARM_TMPL_3NIC_2VM_HA_PARAM.json file is missing." -ErrorAction Stop
}
...
$SLBParamData = Get-Content -Raw -Path ARM_TMPL_3NIC_2VM_HA_SLB_CONFIG_PARAM.json | ConvertFrom-Json -AsHashtable
if ($null -eq $SLBParamData) {
    Write-Error "ARM_TMPL_3NIC_2VM_HA_SLB_CONFIG_PARAM.json file is missing." -ErrorAction Stop
}
