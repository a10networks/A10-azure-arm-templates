...
$ParamData = Get-Content -Raw -Path ARM_TMPL_3NIC_2VM_HA_GLM_PVTVIP_PARAM.json | ConvertFrom-Json -AsHashtable
if ($null -eq $ParamData) {
    Write-Error "ARM_TMPL_3NIC_2VM_HA_GLM_PVTVIP_PARAM.json file is missing." -ErrorAction Stop
}

# Get ARM_TMPL_2NIC_1VM_SLB_CONFIG_PARAM.json file content
$autoParamData = Get-Content -Raw -Path ARM_TMPL_3NIC_2VM_SLB_CONFIG_PARAM.json | ConvertFrom-Json -AsHashtable

if ($null -eq $autoParamData) {
    Write-Error "ARM_TMPL_3NIC_2VM_SLB_CONFIG_PARAM.json data is missing." -ErrorAction Stop
}

$resourceGroupName = $autoParamData.parameters.resourceGroupName
$vThUsername = $autoParamData.parameters.vThUsername
