...
$autoParamData = Get-Content -Raw -Path ARM_TMPL_3NIC_2VM_AUTOMATION_ACCOUNT_PARAM.json | ConvertFrom-Json -AsHashtable

if ($null -eq $autoParamData) {
    Write-Error "ARM_TMPL_3NIC_2VM_AUTOMATION_ACCOUNT_PARAM data is missing." -ErrorAction Stop
}
