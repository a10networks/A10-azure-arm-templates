...
function ConfigureServer {
    ...
    $Body.server.add('health-check-disable', 1) 
...
foreach ($vm_controller in $vms_controller) 
{
    ...
    if($index -eq 0){
        ...
        ConfigureControllerGroup -BaseUrl $vthunderBaseUrl -AuthorizationToken $AuthorizationToken -count $count -controllerip $vm_controller -site $vmNames[$index]
