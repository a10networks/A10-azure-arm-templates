...
class Authentication:
    ...
    @staticmethod
    def get_vth_auth_token(baseurl, username, password):
        """
...
class VMSSvThunders:
    ...
    def list_vmss_vthunders_ips(client, resource_group_name, automation_account_name):
        ...
        vthunder_ip_list = list()
        if isinstance(vth_ips, str):
            if 'null' in vth_ips:
                vth_ips = vth_ips.replace('null', '"null"')
                return vthunder_ip_list
            vth_ips = vth_ips.split(';')
            for each_ip in vth_ips:
                each_ip = each_ip.split('=')[0]
                if each_ip:
                    vthunder_ip_list.append(each_ip)
        return vthunder_ip_list
...
def handle():
    ...
    # get subscription id value
    subscription_id = os.getenv("SubscriptionId")

    # get vThunder user name and password 
    user_name = os.getenv('vThUserName')
    encrypted_password = os.getenv('vThCurrentPassword')
    encryption_key = os.getenv('vThPasswordEncryptionKey')

    # decrypt password
    encryption_key = bytes(encryption_key, 'utf-8')
    encrypted_password = bytes(encrypted_password, 'utf-8')
    secure_password = SecurePassword()
    vth_password = secure_password.decrypt(
