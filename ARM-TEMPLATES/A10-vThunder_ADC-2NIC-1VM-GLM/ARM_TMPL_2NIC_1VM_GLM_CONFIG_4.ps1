...
# Get vthunder 2 nic parameter file content
$ParamData = Get-Content -Raw -Path ARM_TMPL_2NIC_1VM_GLM_PARAM.json | ConvertFrom-Json -AsHashtable
if ($null -eq $ParamData) {
    Write-Error "ARM_TMPL_2NIC_1VM_GLM_PARAM.json file is missing." -ErrorAction Stop
}

$SLBParamData = Get-Content -Raw -Path ARM_TMPL_2NIC_1VM_GLM_SLB_CONFIG_PARAM.json | ConvertFrom-Json -AsHashtable
if ($null -eq $SLBParamData) {
    Write-Error "ARM_TMPL_2NIC_1VM_GLM_SLB_CONFIG_PARAM.json file is missing." -ErrorAction Stop
}
...
#Get Glm parameter file content
$glm_param_data = Get-Content -Raw -Path ARM_TMPL_2NIC_1VM_GLM_CONFIG_PARAM.json | ConvertFrom-Json -AsHashtable
if ($null -eq $glm_param_data) {
    Write-Error "ARM_TMPL_2NIC_1VM_GLM_CONFIG_PARAM.json file is missing." -ErrorAction Stop
}
...
$VmIPName = $ParamData.parameters.publicIPAddressName.value
if ($null -eq $VmIPName) {
    Write-Error "public Ip name is missing." -ErrorAction Stop
}
...
Write-Host "Instance Public IP: "$VmIPAddress

#get baseurl for axapi configuration
$base_url = -join("https://", $VmIPAddress, "/axapi/v3")
...
ActivateLicense -appliance_id $appliance_uuid
Write-Host "ActivateLicense "

ConfigurePrimaryDns -authorization_token $authorization_token -appliance_id $appliance_uuid
