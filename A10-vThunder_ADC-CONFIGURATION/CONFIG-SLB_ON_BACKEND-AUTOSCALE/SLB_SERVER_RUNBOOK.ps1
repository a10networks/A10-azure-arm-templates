...
try {
    # Get variables
    $vnetresourceGroupName = Get-AutomationVariable -Name vnetresourceGroupName -ErrorAction Stop
}
catch {
    Write-Output "Failed to get vnetresourceGroupName variable from Automation Account"
    Write-Error $_
}
try {
...
function GetvThunderIpAdd {
    ...
    try {
        $response = Get-AzPublicIpAddress -Name $hostIPName -ResourceGroupName $vnetresourceGroupName -ErrorAction Stop    
...
foreach ($interface in $mgmtInterfaces) {
    ...
    $hostIPAddress = GetvThunderIpAdd -resourceGroupName $resourceGroupName -vnetresourceGroupName $vnetresourceGroupName -mgmtInterface $interface
    ...
    vth_logout -BaseUrl $vthunderBaseUrl -AuthorizationToken $AuthorizationToken -vthunderIP $hostIPAddress
}
