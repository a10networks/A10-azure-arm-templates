...
$resourceGroup = Read-Host 'Enter resource group name to create'
...
$vnet = New-AzureRmVirtualNetwork -ResourceGroupName $resourceGroup -Location $location -Name "TestVnet" -AddressPrefix 192.168.0.0/16 -Subnet $mgmtsubnet,$data1subnet
...
$vmConfig = Set-AzureRmVMSourceImage -VM $vmConfig -PublisherName "a10networks" -Offer "a10-vthunder-adc" -skus "vthunder_byol" -Version "latest"
$vmConfig = Set-AzureRmVMPlan -Name "vthunder_byol" -Product "a10-vthunder-adc" -Publisher "a10networks" -VM $vmconfig
