...
if ($null -eq $ParamData) {
    Write-Error "vthunder_2nic_parameter_BYOL.json file is missing." -ErrorAction Stop
}

# Get vthunder 2 nic slb parameter file content
...
if ($null -eq $SLBParamData) {
    Write-Error "vthunder_2nic_slb_parameter.json file is missing." -ErrorAction Stop
}
...
Write-Host "vThunder1 Public IP: "$hostIPAddress
