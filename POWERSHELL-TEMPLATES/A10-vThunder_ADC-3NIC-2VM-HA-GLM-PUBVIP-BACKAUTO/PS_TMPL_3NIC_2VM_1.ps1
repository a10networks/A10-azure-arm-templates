...
# Get parameters
$ParamData = Get-Content -Raw -Path PS_TMPL_3NIC_2VM_PARAM.json | ConvertFrom-Json -AsHashtable
...
$vm1Data1NIC = New-AzNetworkInterface -ResourceGroupName $resourceGroup -Name $ParamData.parameters.nic2Name_vm1.value -Location $location -SubnetId $data1Subnet.Id -NetworkSecurityGroupId $vm1nsg.Id
...
if ($null -eq $SLBParamData) {
    Write-Error "PS_TMPL_3NIC_2VM_SLB_CONFIG_PARAM.json file is missing." -ErrorAction Stop
}
...
# Create public IP address for vip
$vipPIp = New-AzPublicIpAddress -ResourceGroupName $resourceGroup -Location $location -AllocationMethod Dynamic -IdleTimeoutInMinutes 4 -Name "vThunderIP$(Get-Random)"

# Add vip is data interface 1
Add-AzNetworkInterfaceIpConfig -Name vip -NetworkInterface $vm1Data1NIC -Subnet $data1Subnet -PrivateIpAddress $vip -PublicIpAddress $vipPIp 
...
$vm1Data2NIC = New-AzNetworkInterface -ResourceGroupName $resourceGroup -Name $ParamData.parameters.nic3Name_vm1.value -Location $location -SubnetId $data2Subnet.Id -NetworkSecurityGroupId $vm1nsg.Id
...
$vm2MgmtSubnet = $vnet.Subnets | Where-Object{ $_.Name -eq $ParamData.parameters.vm1MgmtIntfName.value }
$vm2MgmtNIC = New-AzNetworkInterface -ResourceGroupName $resourceGroup -Name $ParamData.parameters.nic1Name_vm2.value -Location $location -SubnetId $vm2MgmtSubnet.Id -PublicIpAddressId $vm2MgmtPIp.Id -NetworkSecurityGroupId $vm2nsg.Id

$vm2Data1Subnet = $vnet.Subnets | Where-Object{ $_.Name -eq $ParamData.parameters.vm1Eth1Name.value}
$vm2Data1NIC = New-AzNetworkInterface -ResourceGroupName $resourceGroup -Name $ParamData.parameters.nic2Name_vm2.value -Location $location -SubnetId $vm2Data1Subnet.Id -NetworkSecurityGroupId $vm2nsg.Id

$vm2Data2Subnet = $vnet.Subnets | Where-Object{ $_.Name -eq $ParamData.parameters.vm1Eth2Name.value }
$vm2Data2NIC = New-AzNetworkInterface -ResourceGroupName $resourceGroup -Name $ParamData.parameters.nic3Name_vm2.value -Location $location -SubnetId $vm2Data2Subnet.Id -NetworkSecurityGroupId $vm2nsg.Id
