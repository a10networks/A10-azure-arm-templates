...
$resData = Get-Content -Raw -Path PS_TMPL_3NIC_2VM_AUTOMATION_ACCOUNT_PARAM.json | ConvertFrom-Json -AsHashtable
...
# Get PS_TMPL_3NIC_2VM_PARAM parameter file content
$ParamData = Get-Content -Raw -Path PS_TMPL_3NIC_2VM_PARAM.json | ConvertFrom-Json -AsHashtable
if ($null -eq $ParamData) {
    Write-Error "PS_TMPL_3NIC_2VM_PARAM.json file is missing." -ErrorAction Stop
}

# Get PS_TMPL_3NIC_2VM_HA_CONFIG_PARAM parameter file content
$HAParamData = Get-Content -Raw -Path PS_TMPL_3NIC_2VM_HA_CONFIG_PARAM.json | ConvertFrom-Json -AsHashtable
if ($null -eq $HAParamData) {
    Write-Error "PS_TMPL_3NIC_2VM_HA_CONFIG_PARAM.json file is missing." -ErrorAction Stop
}

# Get PS_TMPL_3NIC_2VM_SLB_CONFIG_PARAM parameter file content
