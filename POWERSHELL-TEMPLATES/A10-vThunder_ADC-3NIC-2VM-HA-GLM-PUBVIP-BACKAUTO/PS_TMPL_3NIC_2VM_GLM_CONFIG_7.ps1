...
if ($null -eq $resData) {
    Write-Error "resData is missing" -ErrorAction Stop
}
...
$glm_param_data = Get-Content -Raw -Path PS_TMPL_3NIC_2VM_GLM_CONFIG_PARAM.json | ConvertFrom-Json -AsHashtable

if ($null -eq $glm_param_data) {
    Write-Error "PS_TMPL_3NIC_2VM_GLM_CONFIG_PARAM.json file is missing." -ErrorAction Stop
}
...
# Get PS_TMPL_3NIC_2VM_PARAM.json file content
