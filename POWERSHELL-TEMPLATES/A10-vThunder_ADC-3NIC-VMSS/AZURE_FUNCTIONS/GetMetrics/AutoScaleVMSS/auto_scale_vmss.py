...
def handle():
    ...
    # check scale out threshold less tha scale in threshold
    if scaleout_threshold <= scalein_threshold:
        logging.error('Scale out threshold is less than scale in threshold')
        return
    ...
    try:
        ...
        logging.info('No timestamp cpu data found for last 5 minute')
    ...
    if total_average_cpu < scalein_threshold:
        if current_capacity <= min_scale_in_limit:
            ...
            logging.info('min scale in limit: %s' % min_scale_in_limit)
        else:
            # scale in vmss
            logging.debug('Scaling In VMSS')
            logging.debug({'total_average_cpu': total_average_cpu,
                           'scalein_threshold': scaleout_threshold})
